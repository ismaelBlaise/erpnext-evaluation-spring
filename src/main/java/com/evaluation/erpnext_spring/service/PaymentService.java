...
public class PaymentService {
    ...
    requestBody.put("doc", Collections.singletonMap("name", paymentEntryName));
    ...
    try {
        ...
        if (!response.getStatusCode().is2xxSuccessful()) {
            throw new RuntimeException("Erreur lors de la validation du Payment Entry - HTTP " + 
        ...
        throw new RuntimeException("Erreur côté client (4xx): " + e.getStatusCode() + " - " + 
                e.getResponseBodyAsString(), e);
    } catch (HttpServerErrorException e) {
        throw new RuntimeException("Erreur côté serveur ERPNext (5xx): " + e.getStatusCode() + " - " + 
                e.getResponseBodyAsString(), e);
    } catch (ResourceAccessException e) {
        throw new RuntimeException("Erreur d'accès au serveur ERPNext: " + e.getMessage(), e);
    } catch (Exception e) {
